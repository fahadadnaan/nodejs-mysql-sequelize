...
module.exports = app => {
  //=============================== GET ALL DATA ================================
  app.get("/fridges", async (req, res, next) => {
      await db.fridge_contents.findAll().then((data, err) => {
        if (err)
          res
            .status(500)
            .send({
  ...
  app.get("/fridges/driver", async (req, res) => {
      await db.fridge_contents.findAll({
        include: [db.Driver]
      }).then((data, err) => {
        if (err)
          res
  ...
  app.get("/fridges/:id", async (req, res) => {
      await db.fridge_contents.findByPk(req.params.id).then((result, err) => {
        if (err)
          res
            .status(500)
            .send({
    ...
    async (req, res, next) => {
      ...
      if (!req.is("application/json"))
        ...
        if (!errors.isEmpty()) res.status(422).json({ errors: errors.array() });
          ...
          await db.fridge_contents.create(FridgeContents).then(result =>{
    ...
    async (req, res, next) => {
      ...
      if (!req.is("application/json"))
        ...
        if (!errors.isEmpty()) res.status(422).json({ errors: errors.array() });
          ...
          await db.fridge_contents.update(FridgeContents, {
            where: {
              id: req.params.id
            }
          }).then((result, err) => {            
  ...
  app.delete("/fridges/:id", async (req, res, next) => {
      await db.fridge_contents.destroy({ where: { id: req.params.id } }).then(
        (result, err) => {
          if (err)
            res
              .status(500)
